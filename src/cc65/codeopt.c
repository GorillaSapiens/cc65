...
struct OptFunc {
    ...
    char*          Name;                /* Name of the function/group */
...
#define OPT_LIST \
    ...
    X(OptIndLoads1,      0) \
    X(OptIndLoads2,      0) \
    X(OptJumpCascades, 100) \
    X(OptJumpTarget1,  100) \
    X(OptJumpTarget2,  100) \
    X(OptJumpTarget3,  100) \
    X(OptLoad1,        100) \
    X(OptLoad2,        200) \
    X(OptLoad3,          0) \
    X(OptLongAssign,   100) \
...
#define X(name, factor) \
    static OptFunc D ## name    = { name, #name, factor, 0, 0, 0, 0, 0 };
OPT_LIST
#undef X
