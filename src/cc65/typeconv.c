    ...
    if (ED_IsLVal (Expr)) {
        ...
        */
        if (IsClassFloat (OldType) && IsClassInt (NewType)) {
            long IVal = (long)Expr->FVal;
            if ((Expr->FVal != FP_D_FromInt(IVal).V) && !Explicit) {
                Warning ("Floating point constant (%f) converted to integer loses precision (%ld)",Expr->FVal,IVal);
            }
            Expr->IVal = IVal;
        }
